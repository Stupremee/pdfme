...
The table schema is included in the `@stupremee/schemas` package and is exported as `table`.  
You can add the table schema as a plugin to `@stupremee/ui` and `@stupremee/generator` using the code below.
...
```javascript
import { table } from '@stupremee/schemas';
