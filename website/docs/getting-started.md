...
\*You must install `@stupremee/common` regardless of which package you use.
...
Template Type can be imported by both `@stupremee/generator` or `@stupremee/ui`. Templates are used everywhere.
...
**schemas** can only utilize text by default, but you can load images and various barcodes like QR codes as plugins from the `@stupremee/schemas` package.  
...
By default, examples often demonstrate the use of the `text` schema type. However, you can use other built-in schema types or even create your own custom schemas with the `@stupremee/schemas` package.

#### Step 1: Install `@stupremee/schemas`
