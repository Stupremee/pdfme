  ...
  if (variablesIn) {
    ...
    try {
      ...
      throw new SyntaxError(
        `[@stupremee/schemas] MVT: invalid JSON string '${variablesIn as string}'`,
      );
    }
...
export const validateVariables = (value: string, schema: MultiVariableTextSchema): boolean => {
  ...
  try {
    ...
    throw new SyntaxError(
      `[@stupremee/generator] invalid JSON string '${value}' for variables in field ${schema.name}`,
    );
  }
  ...
  for (const variable of schema.variables) {
    if (!values[variable]) {
      if (schema.required) {
        throw new Error(
          `[@stupremee/generator] variable ${variable} is missing for field ${schema.name}`,
        );
      }
