...
import { cmyk, degrees, degreesToRadians, rgb, Color } from '@stupremee/pdf-lib';
import { Schema, mm2pt, Mode, isHexValid, ColorType } from '@stupremee/common';
...
export const readFile = (input: File | FileList | null): Promise<string | ArrayBuffer> =>
  new Promise((resolve, reject) => {
    ...
    fileReader.onerror = () => {
      reject(new Error('[@stupremee/schemas] File reading failed'));
    };
    ...
    if (file) {
      ...
      reject(new Error('[@stupremee/schemas] No files provided'));
    }
  });
