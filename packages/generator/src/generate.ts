import * as pdfLib from '@stupremee/pdf-lib';
import type { GenerateProps, Schema, PDFRenderProps, Template } from '@stupremee/common';
import {
  checkGenerateProps,
  getDynamicTemplate,
  isBlankPdf,
...
import { getDynamicHeightsForTable } from '@stupremee/schemas';
import {
  insertPage,
  preprocessing,
  postProcessing,
...
const generate = async (props: GenerateProps) => {
  ...
  if (inputs.length === 0) {
    throw new Error(
      '[@stupremee/generator] inputs should not be empty, pass at least an empty object in the array',
    );
  }
